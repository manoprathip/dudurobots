...
-- Customer identity for the DUDU employee/customer pilot.
create table if not exists public.dudu_customers (
  id bigint generated by default as identity primary key,
  ...
  provider text not null default 'pilot_profile',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
create index if not exists dudu_customers_shop_idx on public.dudu_customers(shop);
alter table public.dudu_customers enable row level security;

alter table public.dudu_orders
  add column if not exists customer_id text,
  add column if not exists shop text;
create index if not exists dudu_orders_customer_idx on public.dudu_orders(customer_id, created_at desc);
create index if not exists dudu_orders_shop_idx on public.dudu_orders(shop, created_at desc);
